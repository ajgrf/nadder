...
function readSexp(l: lex.Lexer): lex.StateFn {
  let char = l.nextChar();
  switch (char) {
    case " ":
      l.ignore();
...
let expectedTokens: lex.Token[] = [
  ...
  new lex.Token(lex.TokenType.EOF, ""),
];

describe("Lexer", () => {
  it("should lex simple sexp correctly", () => {
    let input = " (+  foo 1)";
    let lexer = new lex.Lexer(input, readSexp);
